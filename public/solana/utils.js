...
exports.findRaydiumpoolKeys = findRaydiumpoolKeys;
...
exports.getRaydiumPoolsbyMints = getRaydiumPoolsbyMints;
...
function findRaydiumpoolKeys(baseMint, quoteMint, connection) {
...
function getRaydiumPoolsbyMints(connection_1, baseMint_1, quoteMint_1) {
    return __awaiter(this, arguments, void 0, function* (connection, baseMint, quoteMint, limit = 1) {
        try {
            ...
            const response = yield fetchData(msg);
            const data = response.data.data[0];
            let poolKeys;
            let returnPoolKeys = undefined;
            console.log(data);
            if (data.type === `Concentrated`) {
                poolKeys = yield connection.getAccountInfo(new web3_js_1.PublicKey(data.id)).then((item) => (Object.assign({ programId: item.owner, config: data.config, price: data.price, poolId: new web3_js_1.PublicKey(data.id) }, raydium.PoolInfoLayout.decode(item.data))));
                returnPoolKeys = {
                    type: data.type,
                ...
                let resp = yield connection.getAccountInfo(new web3_js_1.PublicKey(data.id));
                let pool = Object.assign({ id: new web3_js_1.PublicKey(data.id), version: 4, programId: resp.owner }, raydium.LIQUIDITY_STATE_LAYOUT_V4.decode(resp.data));
                const market = yield connection.getAccountInfo(pool.marketId).then((item) => (Object.assign({ programId: item.owner }, raydium.MARKET_STATE_LAYOUT_V3.decode(item.data))));
                const authority = raydium.Liquidity.getAssociatedAuthority({
                    programId: raydium.MAINNET_PROGRAM_ID.AmmV4,
                }).publicKey;
...
//   });
// }
function getget(baseMint_1, quoteMint_1) {
    return __awaiter(this, arguments, void 0, function* (baseMint, quoteMint, limit = 100, amount = 10000) {
        try {
